    ...
    ui(new Ui::MainWindow)
{
   ...
   // newCurve->removeFromLegend();
    ...
    ui->customplot5->plotLayout()->insertRow(0);
    QCPTextElement *title5 = new QCPTextElement(ui->customplot5, "yaw profile", QFont("system",15, QFont::Times));
    ui->customplot5->plotLayout()->addElement(0, 0, title5);

    ui->customplot6->plotLayout()->insertRow(0);
    QCPTextElement *title6 = new QCPTextElement(ui->customplot6, "jerk profile", QFont("system",15, QFont::Times));
    ui->customplot6->plotLayout()->addElement(0, 0, title6);
...
//    trajectory_profile p1 , p2 ,p3;
   ...
   profile = plan->get_profile(path,path.size(),sample);
   ...
   for(int i = 0 ; i<100 ;i++){
    ...
    for(int i=0;i<profile.size();i++){
        ui->customplot2->graph(0)->addData(i*sample,profile[i].pos[0]);
        ui->customplot2->graph(1)->addData(i*sample,profile[i].pos[1]);
    }
    ...
    for(int i=0;i<profile.size();i++){
        ...
        ui->customplot3->graph(0)->addData(i*sample,profile[i].vel[0]);
        ui->customplot3->graph(1)->addData(i*sample,profile[i].vel[1]);
    }
    ...
    for(int i=0;i<profile.size();i++){
        ui->customplot4->graph(0)->addData(i*sample,profile[i].acc[0]);
        ui->customplot4->graph(1)->addData(i*sample,profile[i].acc[1]);
    }

    ui->customplot5->addGraph();
    ui->customplot5->graph(0)->setPen(pen);
    pen.setColor(Qt::blue);
    ui->customplot5->graph(0)->setName("angle");
    ui->customplot5->addGraph();
    pen.setColor(Qt::red);
    ui->customplot5->graph(1)->setPen(pen);
    ui->customplot5->graph(1)->setName("velocity");
    ui->customplot5->addGraph();
    pen.setColor(Qt::green);
    ui->customplot5->graph(2)->setPen(pen);
    ui->customplot5->graph(2)->setName("acceleration");

    for(int i=0;i<profile.size();i++){
        ui->customplot5->graph(0)->addData(i*sample,profile[i].yaw[0]);
        ui->customplot5->graph(1)->addData(i*sample,profile[i].yaw[1]);
        ui->customplot5->graph(2)->addData(i*sample,profile[i].yaw[2]);
    }

    ui->customplot6->addGraph();
    ui->customplot6->addGraph();
    pen.setColor(Qt::red);
    ui->customplot6->graph(0)->setPen(pen);
    pen.setColor(Qt::blue);
    ui->customplot6->graph(1)->setPen(pen);
    ui->customplot6->graph(0)->setName("x");
    ui->customplot6->graph(1)->setName("y");
    for(int i=0;i<profile.size();i++){
        ui->customplot6->graph(0)->addData(i*sample,profile[i].jerk[0]);
        ui->customplot6->graph(1)->addData(i*sample,profile[i].jerk[1]);
    }
    ...
    ui->customplot5->setInteractions(QCP::iRangeDrag);
    ui->customplot6->setInteractions(QCP::iRangeDrag);

    ui->customplot->xAxis->setLabel("x");
    ui->customplot2->xAxis->setLabel("time (s)");
    ui->customplot3->xAxis->setLabel("time (s)");
    ui->customplot4->xAxis->setLabel("time (s)");
    ui->customplot5->xAxis->setLabel("time (s)");
    ui->customplot6->xAxis->setLabel("time (s)");
    ...
    ui->customplot5->yAxis->setLabel("angle (rad), vel (rad/s), acc (rad/s/s)");
    ui->customplot6->yAxis->setLabel("jerk (m/s/s/s)");

    ui->customplot2->xAxis->setRange(0,profile.size()*sample);
    ui->customplot2->yAxis->setRange(-2,2);
    ui->customplot3->xAxis->setRange(0,profile.size()*sample);
    ui->customplot3->yAxis->setRange(-4,4);
    ui->customplot4->xAxis->setRange(0,profile.size()*sample);
    ui->customplot4->yAxis->setRange(-6,6);
    ui->customplot5->xAxis->setRange(0,profile.size()*sample);
    ui->customplot5->yAxis->setRange(-8,8);
    ui->customplot6->xAxis->setRange(0,profile.size()*sample);
    ui->customplot6->yAxis->setRange(-10,10);

    ui->customplot2->legend->setVisible(true);
    ui->customplot3->legend->setVisible(true);
    ui->customplot4->legend->setVisible(true);
    ui->customplot5->legend->setVisible(true);
    ui->customplot6->legend->setVisible(true);
